...
(defpackage :cl-cairo2-koans
  (:use :common-lisp)
  (:use :lisp-unit)
  #+sbcl (:use :sb-ext)
  (:export :contemplate))

(in-package :cl-cairo2-koans)
...
(defun load-koan-group-named (koan-group-name)
  ...
  ;; Loads a lisp file at *koan-dir-name* / koan-group-name .lisp
  ;; Adds all the koans from that file to the package.
  (let ((koan-file-name (concatenate 'string (string-downcase (string koan-group-name)) ".lisp")))
...
(defun print-next-suggestion-message ()
  (let ((filename (get-error-filename *collected-results*))
    ...
    (format t " File \"~A/~A.lisp\"~%" *koan-dir-name* (string-downcase filename))
...
(defun print-completion-message ()
  ...
  (format t "**********************************************************~%~%")
  (format t "If you demand greater challenge, take a look at extra-credit.lisp~%")
  (format t "Or, let the student become the teacher:~%")
  (format t " Write and submit your own improvements to github.com/ezrarush/cl-cairo2-koans!~%"))
